...
const TestCases: React.FC = () => {
  ...
  const handleEdit = (testCase: TestCase) => {
    ...
    form.setFieldsValue({
      ...
      target_url: testCase.target_url,
  ...
  const handleExecute = async (testCase: TestCase) => {
    try {
      const response = await api.executeTestCase(testCase.id);
          ...
          <div>
            <Descriptions title="基本信息" bordered column={1} size="small">
              ...
              <Descriptions.Item label="目标URL">
                <a href={selectedTestCase.target_url} target="_blank" rel="noopener noreferrer">
                  {selectedTestCase.target_url}
                </a>
              </Descriptions.Item>
